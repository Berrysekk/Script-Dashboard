...
async def create_role(body: RoleCreateRequest, admin=Depends(require_admin)):
    async with get_db() as db:
        try:
            await auth_service.create_role(db, body.name, body.script_ids, body.category_ids, body.database_ids)
...
async def update_role(role_name: str, body: RoleUpdateRequest, admin=Depends(require_admin)):
    ...
    async with get_db() as db:
        ...
        await auth_service.update_role_databases(db, role_name, body.database_ids)
    return {"ok": True}
